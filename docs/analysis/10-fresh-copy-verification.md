...
Tested commit: `6f40b13` (Phase 4-UI), clone at `/tmp/gmgo-fresh` with no `node_modules`, `dist`,
...
| 1.4 db | `pnpm dev:db:up` | pgvector on :5432 | **failed**: `container name "/gmgo-dev-postgres" is already in use` (second checkout on one machine; the running DB is still reachable) | ⚠️ |
| 1.4 db | `pnpm web db:check` | connected, pgvector installed | `Connected to gmgo_dev as gmgo … pgvector extension: installed` | ✅ |
| 1.4 db | `pnpm db:migrate` | role → migrations → grants | 1.5 s, `Role 'gmgo_app' ready [grants]` | ✅ |
...
| 1.7 CLI (headless) | `GET /auth/cli?redirect_uri=…` with cookie → `GMGO_API_KEY` → `pnpm cli whoami` | — | `User: Olivia Owner <owner@example.test> · Tenant: Acme` | ✅ |
...
| P1 | No headless/agent path for CLI login; `pnpm cli login` opens a browser. | `SETUP.md` §1.7 note + `docs/ADAPTING.md` §3b recipe (use `GET /auth/cli` with a session, or a tenant API key + `GMGO_API_KEY`). |
...
| P1 | Rename checklist missed `gmgo-dev-postgres`/`gmgo-test-postgres` (`container_name`) and `admin@gmgo.local` (seeded global admin, dev quick-login list). | Rows added to `docs/ADAPTING.md` §1, with the second-checkout conflict explained. |
...
tree is now named in `docs/ADAPTING.md` §1 — `@gmgo/{shared,web,cli}` (408), `gmgo_app` (116),
`gm-light`/`gm-dark` (51), `gmgo-starter-*` worker/queue/bucket/workflow names incl. `-staging` and the
commented `-dlq`, `~/.gmgo`, `GMGO_{API_KEY,URL,CONFIG_DIR,DEBUG}`, `GMGO Starter`, `gmgo_dev`/`gmgo_test`/
`gmgo_pass`, `noreply@example.com`, `app/staging.example.com`, `admin@gmgo.local`, container names.
...
passes. `REQUIRE_PROVISIONED=1 pnpm --filter @gmgo/web test:config` fails exactly on
