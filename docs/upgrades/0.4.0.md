...
**Also in this release**, a fix to the kit's own release tooling: `pnpm kit:release` built a
release's CHANGELOG summary from the first *line* of a porting note's "What changed", not its first
*paragraph*. Because notes are written to the repo's 100-column convention, that truncated the
summary mid-clause in the file adopters read to decide whether a release concerns them.
