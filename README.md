...
| Identity: users, tenants, roles, invitations, access requests, magic link, Google/Microsoft OAuth, API keys, admin area | Phase 1 — in progress | `TENANCY_MODE` multi/single, `SIGNUP_MODE`; `GET /auth/cli` handoff for the CLI |
| CLI: `login` (browser → loopback → API key in `~/.gmgo`), `whoami`, `status`, tenant-scoped list commands, `--json` | Phase 1 — in progress | `GMGO_API_KEY` / `GMGO_URL` env overrides for CI |
