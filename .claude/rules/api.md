...
- **Agents read knowledge through `ctx.tools`** (`services/agents/tools/`): `search_knowledge` = `searchChunks` bound to the run's `tenantId`, `get_document` = a tenant-scoped `{ offset, maxChars }` window over `documents.content`; an agent that uses `runToolLoop` includes `ctx.tools`. Add kit-wide tools in `buildAgentTools` (`tools/index.ts`), never by importing `searchChunks` into an agent
