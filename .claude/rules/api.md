...
- Routers: `/api/ai/config` (`aiConfigRouter`: list · `providers` · `readiness` · `test` · upsert · delete), `/api/ai/prompts`, `/api/ai/usage` (`/summary`), `/api/ai/agent-models`, `/api/ai/documents` (`ingest` · `upload` · list · `search` · get · delete), `/api/chat` (conversations + the AG-UI `messages` stream), `/api/agui` (`POST /run` — the `RunAgentInput` endpoint; same `streamChatTurn`), `/api/agents` (registry · `runs` list/create/get/cancel). Permissions: `AiConfig`/`Prompt`/`Document` admin+ `manage`, member `read` (+ `create Document`); `Conversation`/`AgentRun` `manage` for all with ownership route-side (`userId` filter → 404; `isAdminLevel(auth)` widens runs)
