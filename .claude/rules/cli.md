...
# CLI Patterns (`apps/cli`, `@gmgo/cli`, bin `gmgo`)

A commander CLI that talks to the web API with a tenant API key. Dev: `pnpm cli <command>` from the
root (runs `tsx src/cli.ts` inside `apps/cli`); build: `pnpm --filter @gmgo/cli build` → `dist/cli.js` (the `bin`).
...
- `apps/cli/src/api.ts` — the only `fetch` call site (`createApiClient`). Adds `Authorization: Bearer
  <key>`, parses every response with the matching `@gmgo/shared/<module>` schema (errors with the shared
...
- `apps/cli/src/config.ts` — `~/.gmgo/config.json` (`GMGO_CONFIG_DIR` relocates it; tests use a temp
  dir): directory `0700`, file `0600`, re-tightened on every write. Env overrides win over the file:
  `GMGO_API_KEY`, `GMGO_URL` (for CI — no browser); `GMGO_DEBUG` enables debug lines
...
- Never import from `apps/web`; only `@gmgo/shared`, `commander`, `chalk`, `open`, `zod`, `node:*`
...
- Human output goes to stdout via `chalk`; diagnostics and progress to stderr. `--json` on **every
  ...
  into `jq`
- **Never print a full API key.** Show `gmgo_ab12…` (prefix + 4) in `whoami`/`status`/`keys list`;
...
| 2 | `EXIT_NOT_LOGGED_IN` | no key in config/env, or the server answered 401 (`hint`: run `gmgo login`) |
...
(`GMGO_CONFIG_DIR`); call the command function; assert the thrown `CliError.exitCode` and the parsed
`--json` output. Never touch the real `~/.gmgo`.
