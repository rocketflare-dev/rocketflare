...
| `plugin check`: a barrel `has no line for it` / `names it, but … is not there` | the five barrels and the plugin's directories disagree | `/rf-plugin` — `pnpm plugin add`/`remove` writes those lines; never hand-edit a barrel to make the check pass |
