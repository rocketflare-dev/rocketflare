...
| 4 | `security-headers.ts` | Headers after `next()` | Early so 4xx/5xx from later middleware are covered too |
| 5 | `body-limit.ts` | 1 MB cap on `/api/*`, `/auth/*` → 413 | Before any parsing or DB work |
